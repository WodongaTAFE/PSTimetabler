function New-CTFaculty {
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess($Name, 'Create faculty.')) {
            (Invoke-RestMethod -Uri $uri -Headers $headers -Method Post -Body (ConvertTo-Json $body) -ContentType 'application/json') | Add-Member -MemberType AliasProperty -Name FacultyID -Value Id -PassThru 
        }
    }
}